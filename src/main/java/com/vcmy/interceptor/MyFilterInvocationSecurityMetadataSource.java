...
public class MyFilterInvocationSecurityMetadataSource implements FilterInvocationSecurityMetadataSource {
    ...
    public Collection<ConfigAttribute> getAttributes(Object o) {
        ...
        //去数据库查询资源
        List<RolePermission> permissionList = permissionDao.getAllPermission();//查出所有权限+角色的组合
        ...
        for (RolePermission permission : permissionList) {
            if (!StringUtils.isBlank(permission.getUrl())){
                ...
                for(String url:urls){
                    if (!StringUtils.isBlank(url) && antPathMatcher.match(url, requestUrl)
                            && permission.getRoleId()!=null && permission.getRoleId()>0) {
                        rolesId.add(permission.getRoleId());
                    }
                }
            }
        }
        String[] values = new String[rolesId.size()];
        for (int i = 0; i < rolesId.size(); i++) {
            values[i] = rolesId.get(i).toString();
        }
        if(values != null && rolesId.size() != 0){
