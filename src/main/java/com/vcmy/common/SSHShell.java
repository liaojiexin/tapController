...
public final class SSHShell {
	...
	private List<Match> bulidPattern(StringBuilder buffer,Integer sshTimeout) throws MalformedPatternException {
		List<Match> lstPattern = new ArrayList<>();
		
    //	String[] regEx = {"~\\$","\\<R.*\\>","\\[R.*\\]" }; //h3c路由器                  "\\[Router.*\\]" 
//    	String[] regEx = {"~\\$","\\<H.*\\>","\\[H.*\\]","OVS\\$","\\<R.*\\>","\\[R.*\\]"}; 	 //之前h3c交换机
//    	String[] regEx = { "#", "~#", "~\\$","\\<H.*\\>","\\[H.*\\]" };
//    	String[] regEx = { "\\>", "#", "~#", "~\\$" };
//    	String[] regEx = { "netconf\\>", "#", "~#", "~\\$" ,"password:"};
		String[] regEx = {"~#"};
	...
	private SSHShell(String host, int port, String userName, String password) throws JSchException, IOException {
		try {
			linuxPromptMatches = bulidPattern(shellBuffer, 10000);
	...
	public String runCommands(List<String> commands) throws Exception {
		...
		try {
			...
			System.out.println("打印命令：*********"+output);
		} finally {
	 ...
	 *             exception thrown
	 */
	public String runNewCommands(List<String> commands) throws Exception {
		String output = null;
		try {
			for (String command : commands) {
				expect.expect(this.linuxPromptMatches);
...
//				System.out.println("expect.expect(this.linuxPromptMatches): "+expect.expect(this.linuxPromptMatches));
			}
			expect.expect(this.linuxPromptMatches);
			output = shellBuffer.toString();
//			System.out.println("打印命令：************************************************************************"+output);
